...
bool GUserPlayStatus(sockaddr_in user) {
	list<User>::iterator it = userList.begin();
	for (size_t i = 0; i < userList.size(); i++) {
		if (it->_userip.sin_addr.S_un.S_addr == user.sin_addr.S_un.S_addr && it->_userport.sin_port == user.sin_port) {
...
bool FindEmptyRoom(sockaddr_in user) {
	...
	list<User>::iterator it = userList.begin();
	for (size_t i = 0; i < playRooms.size(); i++) {
		if (playRooms[i]._playerX == "empty" && playRooms[i]._playerO != GUserName(user)) {
		...
		else if (playRooms[i]._playerO == "empty" && playRooms[i]._playerX != GUserName(user)) {
...
string DrawBoard(PlayRoom selectedroom) {
	...
	/*
	completeGrid = selectedroom.spaces[0][0] + selectedroom.spaces[0][1] + selectedroom.spaces[0][2] + '\n' +
		selectedroom.spaces[1][0] + selectedroom.spaces[1][1] + selectedroom.spaces[1][2] + '\n' +
		selectedroom.spaces[2][0] + selectedroom.spaces[2][1] + selectedroom.spaces[2][2] + '\n';*/
	return completeGrid;
}

//From a 0 to 10 of hardcoding I give it a 53
...
void PlayerMoveMaker(sockaddr_in user, char place) {
	for (size_t i = 0; i < playRooms.size(); i++) {
		if (playRooms[i]._playroomname == GetPlayerGameRoom(user)._playroomname) {
			switch (place) {
			case '1':
				if (GUserName(user) == playRooms[i]._playerX)
					playRooms[i].spaces[0][0] = 'X';
				else if (GUserName(user) == playRooms[i]._playerO)
			...
			if (result == "none"){
				...
				WriteServerAnswer(DrawBoard(GetPlayerGameRoom(user)) + "\nNo more moves to make! It's a DRAW\n");
			}
			else if (result != "draw" && result != "none"){
...
void Command(string _buf, sockaddr_in user) {
	...
	//#play
		...
		if (FindEmptyRoom(user)){
			...
			else if (GUserName(user) == GetPlayerGameRoom(user)._playerO && GetPlayerGameRoom(user)._playerX == "empty"){
				WriteServerAnswer("A gameroom has been found! But you are alone, so you'll have to wait for another player\n");
			}
			else if (GUserName(user) == GetPlayerGameRoom(user)._playerX && GetPlayerGameRoom(user)._playerO != "empty"){
...
void PlayCommand(string _buf, sockaddr_in user) {
	...
	//#quit
		...
		if (GUserName(user) == GetPlayerGameRoom(user)._playerX){
		...
		else if (GUserName(user) == GetPlayerGameRoom(user)._playerO){
	...
	//#"Number of placing"
		...
		if (!GetPlayerGameRoom(user)._gameEnded){
			if (GUserName(user) == GetPlayerGameRoom(user)._playerX && GetPlayerGameRoom(user)._isXTurn == true) {
				PlayerMoveMaker(user, _buf[1]);
			}
			else if (GUserName(user) == GetPlayerGameRoom(user)._playerO && GetPlayerGameRoom(user)._isXTurn == false){
				PlayerMoveMaker(user, _buf[1]);
			}
			else {
				WriteServerAnswer("It's not your turn " + GUserName(user) + '\n');
			}
		}
		else{
	...
	while (1) {
		...
		if (n>0) {
			...
			if (CheckUserIdentity(si_other)) {
				if (buf[0] == '#') {
					if (GUserPlayStatus(si_other))
						PlayCommand(buf, si_other);
					else
