		...
		if (playRooms[i]._playroomname == GetPlayerGameRoom(user)._playroomname) {
			...
			case '1':
				if (playRooms[i].spaces[0][0] == '1') {
					...
					WriteServerAnswer("No changing whats written! Lose your turn \n");
				}
				break;
...
void Command(string _buf, int user) {
	...
	//#play
		...
		else {WriteServerAnswer("A gameroom has been found! But you are alone, so you'll have to wait for another player\n");}
	}
	else {
...
int main() {
	...
	struct timeval tv;
	int n;
	//Make the server wait for calls
	while (true) {
